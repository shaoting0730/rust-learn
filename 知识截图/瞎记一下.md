...
栈： 后进先出，系统自动分配内存。
堆： 先进先出，程序员管理内存。<br/>
46、版本号中的0.8.5 事实上是 ^0.8.5 的简写，它表示任何至少是 0.8.5 但小于 0.9.0 的版本。<br/>
47、语句（Statements）是执行一些操作但不返回值的指令。 表达式（Expressions）计算并产生一个值。<br/>
47、Rust 不允许自身或其任何部分实现了 Drop trait 的类型使用 Copy trait。<br/>
48、不用获取所有权就可以使用值的功能，叫做 引用（references）。<br/>
49、借用与引用：Rust提供了一个不用获取所有权就可以使用值的功能，叫做 引用（references） ，将创建一个引用的行为称为 借用（borrowing），不能 **同时** 拥有可变引用和不可变引用，但是可以拥有多个不可变引用。  <br/>  
50、数据竞争： 数据竞争（data race）类似于竞态条件，它可由这三个行为造成： <br/>
* 两个或更多指针同时访问同一数据。 <br/>
* 至少有一个指针被用来写入数据。 <br/>
* 没有同步数据访问的机制。    <br/><br/>

51、切片（slice）允许你引用集合中一段连续的元素序列，而不用引用整个集合。slice 是一种引用，所以它不拥有所有权。字符串字面值就是 slice。
52、字符串字面值就是 slice：

```
 let my_string_literal = "hello world";  // 字符串字面值就是 slice
```

这里 my_string_literal 的类型是 &str：它是一个指向二进制程序特定位置的 slice。这也就是为什么字符串字面值是不可变的；&str 是一个不可变引用。 <br/>
53、结构体（struct），或者 structure，是一个自定义数据类型，允许你包装和命名多个相关的值，从而形成一个有意义的组合 <br/>
元组结构体：如
...
54、方法语法 <br/>
方法（method）与函数类似：它们使用 fn 关键字和名称声明，可以拥有参数和返回值，同时包含在某处调用该方法时会执行的代码。不过方法与函数是不同的，因为它们在结构体的上下文中被定义(或者是枚举或 trait 对象的上下文),并且它们第一个参数总是 self，它代表调用该方法的结构体实例。<br/>
```
truct Rectangle {
 width: u32,
 height: u32,
...
```
所有在 impl 块中定义的函数被称为 关联函数（associated functions），因为它们与 impl 后面命名的类型相关.每个结构体都允许拥有多个 impl 块。 <br/>
55、枚举，和结构体类似，结构体给予你将字段和数据聚合在一起的方法，而枚举给予你一个途径去声明某个值是一个集合中的一员。<br/>
56、match 控制流结构: Rust 有一个叫做 match 的极为强大的控制流运算符,它允许我们将一个值与一系列的模式相比较，并根据相匹配的模式执行相应代码。模式可由字面值、变量、通配符和许多其他内容成 。<br/>
57、使用包（package）、Crate 和模块管理不断增长的项目：<br/>
一个包（package）可以包含多个二进制 crate项和一个可选的库 crate。 <br/>
• 包（Packages）：Cargo 的一个功能，它允许你构建、测试和分享 crate。 <br/>
• Crates ：一个模块的树形结构，它形成了库或可执行文件项目。 <br/>
• 模块（Modules）和 use：允许你控制作用域和路径的私有性。 <br/>
• 路径（path）：一个为例如结构体、函数或模块等项命名的方式。 <br/>
58、常见集合 <br/>
 向量(Vector)允许我们一个挨着一个储存一系列数量可变的值. <br/>
 字符串(String)是字符的集合. <br/>
 哈希 map（hash map）允许我们将值与一个特定的键（key）相关联。这是一个叫做 map的更通用的数据结构的特定实现. <br/>
59、Vector <br/>
 Vec<T> ，也被称为 vector。vector 允许我们在一个单独的数据结构中储存多于一个的值，它在内存中彼此相邻地排列所有的值。vector 只能储存相同类型的值。 <br/>
新建 Vector <br/>
```
let v: Vec<i32> = Vec::new();
```
...
```
通过索引获取值是最简单方便的方法,而通过get方法会获取到一个Option.安全性更高.  <br/>
遍历 vector 中的元素  <br/>
...
使用枚举来储存多种类型,vector 只能储存相同类型的值,可以巧妙:
...
```
丢弃 vector 时也会丢弃其所有元素, 类似于任何其他的 struct ，vector 在其离开作用域时会被释放
```
{
let v = vec![1, 2, 3, 4];
// 使用 v
...
```
60、字符串String <br/>
字符串（String ）类型由 Rust 标准库提供，而不是编入核心语言，它是一种可增长、可变、可拥有、UTF-8 编码的字符串类型。提及 Rust 中的 “字符串 “时，他们可能指
的是 String 或 string slice &str 类型，而不仅仅是其中一种类型,但这两种类型在 Rust 的标准库中都有大量使用，而且 String 和 字符串 slices 都是
UTF-8 编码的.<br/>
新建字符串<br/>
```
  let mut s = String::new();
  let s = "initial contents".to_string();  //  该方法也可直接用于字符串字面值： 
  let s = String::from("initial contents");  // 使用 String::from 函数从字符串字面值创建 String
```
更新字符串<br/>
使用 push_str 和 push 附加字符串
```
mut s1 = String::from("foo");
 let s2 = "bar";
 s1.push_str(s2);
 println!("s2 is {s2}");   //     有效
...
```
索引字符串 <br/>
String 是一个 Vec<u8> 的封装。最好是转换为char再进行操作.
```
let s1 = String::from("hi");
let h = s1[0];  // 错误 the type `str` cannot be indexed by `{integer}`
// 字符
for c in "Зд".chars() {
 println!("{c}");  //  З   д
}
...
```
61 、 哈希 map <br/>
哈希 map（hash map）、HashMap<K, V> 类型储存了一个键类型 K对应一个值类型 V 的映射。它通过一个哈希函数（hashing function）来实现映射，决定如何
将键和值放入内存中。很多编程语言支持这种数据结构，不过通常有不同的名字：哈希、map、对象、哈希表、字典或者关联数组，仅举几例.
```
use std::collections::HashMap;
let mut scores = HashMap::new();
scores.insert(String::from("Blue"), 10);
...
```
对于像 i32 这样的实现了 Copy trait 的类型，其值可以拷贝进哈希 map。对于像 String 这样
拥有所有权的值，其值将被移动而哈希 map 会成为这些值的所有者
```
use std::collections::HashMap;
let field_name = String::from("Favorite color");
let field_value = String::from("Blue");
...
expect
   let greeting_file = File::open("hello.txt").expect("改文件打不了");
```
